  ...
  gtsam::Vector RobotBetweenFactor::evaluateError(const gtsam::Pose2& pose1, const gtsam::Pose2& pose2,
      ...
      gtsam::Pose2 relativePose = pose2.between(pose1);
      gtsam::Vector3 error;

      // Calculate the error as the difference between the measured and predicted values
      error(0) = between_.x() - relativePose.x();
      error(1) = between_.y() - relativePose.y();
      error(2) = between_.theta() - relativePose.theta();

      // Calculate the Jacobians
      gtsam::Matrix H1_local, H2_local;
      relativePose = pose2.between(pose1, H2_local, H1_local);
      if (H1) {
          *H1 = H1_local;
      }
      if (H2) {
